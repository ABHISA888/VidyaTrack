...
const AppContent = () => {
  ...
  const { data, loading, error } = useASERData();
  ...
  if (loading) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-gray-50">
        ...
        <p className="font-syne font-extrabold uppercase tracking-[0.4em] text-navy text-xs animate-pulse">Initializing VidyaTrack Intelligence</p>
      </div>
    );
  }
      ...
      <div className="h-screen w-full flex flex-col items-center justify-center bg-red-50 p-10">
        <div className="bg-white p-10 rounded-3xl shadow-2xl border-4 border-red-200 max-w-xl text-center">
           ...
           <p className="text-gray-500 font-medium mb-8 leading-relaxed">
             We encountered a critical error while loading the ASER intelligence dataset. Ensure the data file exists and contains the required schema.
           </p>
           ...
           <button 
             ...
             className="w-full bg-navy text-white px-6 py-4 rounded-xl font-bold font-syne text-[10px] uppercase tracking-[0.2em] shadow-lg hover:bg-navy/90 hover:translate-y-[-2px] transition-all"
           >
